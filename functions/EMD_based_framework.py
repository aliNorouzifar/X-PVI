...
def generate_features(bin_size,w,order):
    ...
    event_table['case_id'] = pd.Categorical(event_table['case_id'], categories=order, ordered=True)
...
def sliding_window(bins, n_bin, sensitivity):
...
def plot_figures(df, masks, n_bin, map_range, dist_matrix, peaks, constraints, w, cluster_bounds,clusters_with_declare_names, data_color, corr_mat):
    ...
    return f'data:image/png;base64,{fig_data1}', f'data:image/png;base64,{fig_data2}', f'data:image/png;base64,{fig_data3}', f'data:image/png;base64,{fig_data4}'
...
################### Explainability ######################################
    ...
    fig1_path, fig2_path, fig3_path, fig4_path = plot_figures(df, masks, n_bin, map_range, dist_matrix, peaks,constraints, w, cluster_bounds, clusters_with_declare_names,data_color, corr_mat)
